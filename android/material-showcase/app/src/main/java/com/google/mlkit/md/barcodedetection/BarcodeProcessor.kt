...
class BarcodeProcessor(
    ...
    private val onBarcodeDetected: (Barcode) -> Unit // Added callback parameter
) : FrameProcessorBase<List<Barcode>>() {
        ...
        if (barcodeInCenter == null) {
            ...
            if (sizeProgress < 1) {
                ...
                if (PreferenceUtils.shouldDelayLoadingBarcodeResult(graphicOverlay.context)) {
                    ...
                    workflowModel.setWorkflowState(WorkflowState.DETECTED)
                    workflowModel.detectedBarcode.setValue(barcodeInCenter)
                    // Call the callback function when a barcode is detected
                    onBarcodeDetected(barcodeInCenter)
                }
            }
        }
        graphicOverlay.invalidate()
