...
class BarcodeSearchActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Set the video to be the default recorded video if available
        if (videoUri != null) {
            playDefaultVideo(videoUri!!)
        }
    ...
    private fun playDefaultVideo(videoUri: Uri) {
        // Set up the VideoView with the video Uri
        videoView.setVideoURI(videoUri)
        ...
        videoView.setOnPreparedListener { mediaPlayer ->
            try {
                mediaPlayer.isLooping = false  // Ensure video only plays once
                videoView.pause()  // Pause initially so that the user has to press play manually

                // Inform the user that the video is ready to be played
                Toast.makeText(
                    this,
                    "Video is ready. Tap the play button to start.",
                    Toast.LENGTH_SHORT
                ).show()

            } catch (e: Exception) {
                Log.e("BarcodeSearchActivity", "Error while preparing video: ${e.message}")
                Toast.makeText(this, "Error playing video, please try again", Toast.LENGTH_SHORT).show()
            }
        }
    }
